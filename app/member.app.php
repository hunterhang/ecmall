...
class MemberApp extends MemberbaseApp
{
    ...
    function register()
    {
    	if(isset($_GET['t']))
    	{
    		$user_id= base_convert($_GET['t'],16,10);
    		$model_member=& m('member');
    	    $find_data = $model_member->get("user_id =".$user_id);
    	    if(!empty($find_data))
    	    {
    	     $_SESSION['tuijian_id']=$user_id;
    	    }
    	}
        ...
        if (!IS_POST)
        {
           ...
           if(strpos($agent,"MicroMessenger")) {
            $this->display('wxmember.register.html');
           }else 
           {
        ...
        else
        {
         ...
         {
         	$this->show_message('注册成功。','back_list','index.php?app=store&id='.$_SESSION['wx_store_id']);
         }else{
