...
class FrontendApp extends ECBaseApp
{
			...
			if(!empty($_GET['wx_store_id']))
		{
				...
				if($this->visitor->get('user_id')=='')
			{
				...
				if(!empty($_GET['wecha_id']))
           	{
           		...
           		$this->wx_register($_GET['wecha_id']);
           		$this->wx_store_id($_GET['token']);
           	}
			else
		     {
				...
				$model_wechat=& m('wechat');
    		     ...
    		     $find_data = $model_wechat->get("user_id='".$_GET['wx_store_id']."'");
				
    		    if(is_array($find_data))
    		 {
                $_SESSION['wxstore_id']=$find_data['user_id'];
				$_SESSION['wx_store_id']=$_GET['wx_store_id'];
				if($find_data['wx_type']==1)
				{
		        $return_url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
				//echo rawurlencode($return_url);exit;
                $model_wxconfig=& m('wxconfig');
    		    $find_data = $model_wxconfig->get("user_id='".$_GET['wx_store_id']."'");
			    $appid=$find_data['appid'];
				$REDIRECT_URI=SITE_URL."/index.php?app=aouth";
				$url='	https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.$REDIRECT_URI.'&response_type=code&scope=snsapi_userinfo&state='.$_GET['wx_store_id'].','.rawurlencode($return_url).'#wechat_redirect';
    ...
    function get_store_id()
    {
      ...
      {
      	$_SESSION['wx_store_id']=$_GET['wx_store_id'];
      	$_SESSION['wxstore_id']=$_GET['wx_store_id'];
      }
    }
    
    function wx_store_id($token)
    {
    	if(!empty($token))
    	{
    		$model_wechat=& m('wechat');
    		$find_data = $model_wechat->get("token='".$token."'");
    		if(is_array($find_data))
    		{
            $_SESSION['wxstore_id']=$find_data['user_id'];
    		}
    ...
    function wx_register($wecha_id)
    {
    	    $ms =& ms(); //连接用户中心 
    	    if($ms->user->check_username($wecha_id))//未注册
    	    {
    	    $user_id = $ms->user->register($wecha_id,'','');
    	     $user_id = $ms->user->auth($wecha_id, '');
    ...
    function login()
    {
        ...
        else
        {
         ...
         if($_SESSION['wx_store_id'])
         {
         	$this->show_message('登入成功。','back_list','index.php?app=store&id='.$_SESSION['wx_store_id']);
         }else{
    ...
    function _do_login($user_id)
    {
        ...
        $user_info = $mod_user->get(array(
            ...
            'fields'        => 'user_id, user_name,real_name, reg_time, last_login, last_ip, store_id',
        ));
 ...
 function wx_do_login($user_id,$info)
    {
        ...
        /* 更新用户登录信息 */
        $mod_user->edit("user_id = '{$user_id}'", "last_login = '" . gmtime()  . "', last_ip = '" . real_ip() . "', real_name = '".$info['real_name']. "', portrait='".$info['portrait']."', logins = logins + 1");
        ...
        $menu['my_ecmall'] = array(
            ...
            'submenu'   => array(
              ...
              /*  'friend'  => array(
                 ...
                 'alipay'  => array(
                    'text'  => '充值管理',
                    'url'   => 'index.php?app=alipay',
                    'name'  => 'alipay',
                    'icon'  => 'ico5',
                ),
                 'make_money'  => array(
                    'text'  => '赚钱系统',
                    'url'   => 'index.php?app=make_money',
                    'name'  => 'make_money',
                    'icon'  => 'ico5',
                ),
     ...
     /*   $menu['im_buyer'] = array(
        ...
        $menu['wechat'] = array(
            ...
            'submenu'   => array(
                'wechat_index'  => array(
                    'text'  =>'微信接口管理',
                    'url'   => 'index.php?app=wechat',
                    'name'  => 'wechat_index',
                    'icon'  => 'ico100',
                ),
                'follow_index'  => array(
                    'text'  =>'关注自动回复',
                    'url'   => 'index.php?app=wechat&act=follow_index',
                    'name'  => 'follow_index',
                    'icon'  => 'ico101',
                ),
                 'keyword_index'  => array(
                    'text'  =>'关键词自动回复',
                    'url'   => 'index.php?app=wechat&act=keyword_index',
                    'name'  => 'keyword_index',
                    'icon'  => 'ico102',
                ),
                 'message_index'  => array(
                    'text'  =>'消息自动回复',
                    'url'   => 'index.php?app=wechat&act=message_index',
                    'name'  => 'message_index',
                    'icon'  => 'ico103',
                ),
                 'custom_menu'  => array(
                    'text'  =>'自定义菜单',
                    'url'   => 'index.php?app=custom_menu',
                    'name'  => 'custom_menu',
                    'icon'  => 'ico104',
                ),
...
class StorebaseApp extends FrontendApp
{
  ...
  function get_store_data()
     ...
     $cache_server =& cache_server();
       $key = 'function_get_store_data_' . $this->_store_id;
        ...
        if ($store == false)
        {
			...
			$store['store_gcates_image']=$this->_get_store_gcategory_images();
	...
	function _get_store_gcategory_images()
