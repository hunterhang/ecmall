...
.f_l{float:left;}
.sub_btn {background:transparent url(<?php echo $this->res_base . "/" . 'images/member/btn.gif'; ?>) no-repeat scroll 0 -253px; border:0 none; color:#fff; cursor:pointer; font-weight:bold; height:32px; width:120px; }
...
.pic3{display: block; line-height: 40px; padding-left: 40px; font-size: 14px; font-weight: bold; color: #fff; background: url(<?php echo $this->res_base . "/" . 'images/s_down.png'; ?>) no-repeat 10px center #3F89EC;}
</style>
...
<div class="content">
  ...
  <div id="right"> <?php echo $this->fetch('member.submenu.html'); ?>
    ...
    <p style="padding:0 10px 10px 20px; font-size:15px;">在您使用4开头的淘宝助理前，如果您不懂淘宝助理的使用，请仔细阅读<a href="<?php echo $this->res_base . "/" . 'images/zhulijiaocheng.jpg'; ?>" target="_blank" class="btn1 downs" style="display: inline-block;width:165px; margin-left:10px;"><span class=" pic3">淘宝助理使用教程</span></a></p>
    <p style="padding:0 10px 10px 20px; font-size:15px;">并对照检查。电脑上请保证有Excel办公软件。</p>
      <div class="public">
        <form method="post" enctype="multipart/form-data" id="goods_form">
            ...
            第一步：导入CSV文件&nbsp;&nbsp;<b style="color:red;font-size:14px;">请使用淘宝助理4开头的版本，4.0-4.7均可</b>
          
            <?php if ($this->_var['step'] == 2): ?><label class="error right">OK!<?php endif; ?></label><a title="close" class="fold <?php if ($this->_var['step'] == 2): ?>span_open<?php else: ?>span_close<?php endif; ?>"></a>
          </h2>
          ...
          <h2 class="title">第二步：上传商品图片<label style="display:none" class="error right">OK!</label><a title="close" class="fold <?php if ($this->_var['step'] == 2): ?>span_close<?php else: ?>span_open<?php endif; ?>"></a>
          </h2>
